...
try:
    ...
    def extract_data_for_bscore(today, obs_period=6):
        if today.day == 1:
            ...
            sql_script = """
            SELECT s.app_source, s.product, s.pos_dt, s.acc_no, s.id_no, s.mob, s.out_0m_b, s.out_1m_b, s.out_2m_b, s.out_3m_b, s.out_4m_b, 
                    s.out_5m_b, s.limit, s.interest_rate, s.installment_amount, s.first_payment_default_flag, s.first_three_payments_default_flag, 
                    s.dlq_class, s.max_dpd_l5m_b, s.dpd,
                    r.salary,r.caractualprice, r.carname, r.motorcycletype, r.age, r.gender, r.education, r.occupation, r.profess, r.marriage, r.city, 
                    r.work_province, r.car_name, r.carBrand, r.carAge, r.if_buy_back, r.loan_value_date, r.tenor, r.loan_maturity_date
            FROM `std_m_ldr_acc` as s
            LEFT JOIN `wcd car_loan_ldr` as r
            ON (s.acc_no = r.agreement_no AND s.pos_dt = r.batch_id)
            WHERE s.pos_dt = date('%s') 
              AND s.app_source = 'wcd'
    ...
    def behavioral_factors(df, today):       
        ...
        df5['current_debt_burden'] = df5['monthly_repayment_(PBOC)'] / df5['salary']
    ...
    def grad(json_temp):
        ...
        for i in var_list_std:
            if not isinstance(ldr.iloc[0][i], str):
                ...
                ldr["rccr_"+i]=np.NaN
                for j in fmt2.index:
                    #ldr.loc[(ldr[i]>=fmt2.loc[j][min_c])&(ldr[i]<=fmt2.loc[j][max_c]),["rccr_%s"  % i]]=fmt2.loc[j,['rccr']].values.tolist()
    ...
    if (today.day == 1):
        ...
        if rerun_flag == "yes":
            with engine.begin() as conn:
                sql_script = (""" DELETE FROM `b_sc_acc` WHERE pos_dt = date('%s') and app_source = 'wcd' and product='car_loan' """% yesterday.strftime("%Y-%m-%d") )
                print("Time:", datetime.datetime.now().strftime("%H:%M:%S"))
                print(sql_script)
                conn.execute(sql_script)
                print("Time:", datetime.datetime.now().strftime("%H:%M:%S"))
                
                sql_script = (""" DELETE FROM `wcd car_loan_bscore_factor` WHERE pos_dt = date('%s') """% yesterday.strftime("%Y-%m-%d") )
                print("Time:", datetime.datetime.now().strftime("%H:%M:%S"))
                print(sql_script)
                conn.execute(sql_script)
                print("Time:", datetime.datetime.now().strftime("%H:%M:%S"))

                sql_script = (""" DELETE FROM `wcd car_loan b champ 20201026` WHERE pos_dt = date('%s') """% yesterday.strftime("%Y-%m-%d") )
                print("Time:", datetime.datetime.now().strftime("%H:%M:%S"))
                print(sql_script)
                conn.execute(sql_script)
                print("Time:", datetime.datetime.now().strftime("%H:%M:%S"))
        ...
        if not json_temp.empty:
            ...
            with engine.begin() as conn:
                ...
                df10.to_sql(name='wcd car_loan_bscore_factor', con=conn, chunksize=10000, if_exists='append', index=False, method='multi')#'replace'
                print("tables: b_sc_acc, wcd car_loan_bscore_factor, ", table_name)

    print("================================")
    print("Time:", datetime.datetime.now().strftime("%H:%M:%S"))
    print("E01_2_1_factor_wcd.py: Completed")
    print("================================")
    running_flag = False
    thr1.join()
    sys.exit(0)
    ...
    print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
    print("Time:", datetime.datetime.now().strftime("%H:%M:%S"))
    strLog = "Failed to process E01_2_1_factor_wcd.py {}".format(error)
    print(strLog)
    print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
    running_flag = False
    thr1.join()
