...
try:
    ...
    def behavioral_factors(df, today):       
        ...
        df5['mob_to_term'] = df5['mob'] / round(df5['tenor']/30,0)
        ...
        df5['act_to_sched_out_current'] = df5['act_to_sched_out_0m_b']
        df9 = df5.copy()
    ...
    if (today.day == 1):
        ...
        if rerun_flag == "yes":
            with engine.begin() as conn:
                sql_script = (""" DELETE FROM `b_sc_acc` WHERE pos_dt = date('%s') and app_source = 'xbd' and product='per_com_loan' """% yesterday.strftime("%Y-%m-%d") )
                print("Time:", datetime.datetime.now().strftime("%H:%M:%S"))
                print(sql_script)
                conn.execute(sql_script)
                print("Time:", datetime.datetime.now().strftime("%H:%M:%S"))
                
                sql_script = (""" DELETE FROM `xbd per_com_loan_bscore_factor` WHERE pos_dt = date('%s') """% yesterday.strftime("%Y-%m-%d") )
                print("Time:", datetime.datetime.now().strftime("%H:%M:%S"))
                print(sql_script)
                conn.execute(sql_script)
                print("Time:", datetime.datetime.now().strftime("%H:%M:%S"))
                
                sql_script = (""" DELETE FROM `xbd per_com_loan b champ 20210609` WHERE pos_dt = date('%s') """% (yesterday.strftime("%Y-%m-%d")) )
                print("Time:", datetime.datetime.now().strftime("%H:%M:%S"))
                print(sql_script)
                conn.execute(sql_script)
                print("Time:", datetime.datetime.now().strftime("%H:%M:%S"))
        ...
        if not json_temp.empty:
            ...
            json_temp["app_source"] = "xbd"
            json_temp["product"] = "per_com_loan"
            ...
            with engine.begin() as conn:
                ...
                print('before xbd per_com_loan_bscore_factor')
                df10.to_sql(name='xbd per_com_loan_bscore_factor', con=conn, chunksize=10000, if_exists='append', index=False, method='multi')#'replace'
                print("tables: b_sc_acc, xbd per_com_loan_bscore_factor, ", table_name)

    print("================================")
    print("Time:", datetime.datetime.now().strftime("%H:%M:%S"))
    print("E06_1_1_factor_xbd.py: Completed")
    print("================================")
    running_flag = False
    thr1.join()
    sys.exit(0)
    ...
    print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
    print("Time:", datetime.datetime.now().strftime("%H:%M:%S"))
    strLog = "Failed to process E06_1_1_factor_xbd.py {}".format(error)
    print(strLog)
    print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
    running_flag = False
    thr1.join()
