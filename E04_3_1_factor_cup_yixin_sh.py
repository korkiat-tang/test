...
try:
        ...
        if today.day == 1:
            ...
            sql_script = """
                SELECT  s.pos_dt, s.acc_no, s.id_no, s.mob, 
                        ...
                        cast(j.appraisal_price as SIGNED) as caractualprice,
                ...
                ON r.app_no = j.app_no
                WHERE s.pos_dt = date('%s') 
                AND s.product = 'car_loan'
                AND s.app_source = 'CUP_YIXIN_SH'
    ...
    def behavioral_factors(df, today):       
        ...
        #print("222 %s " % df7[['acc_no','carAge', 'car_age_ori', 'caractualprice', 'car_age_current']])

        for i in range(df7.shape[0]):
            #print("i is %s"%i)
            #print("1 %s " % df7.loc[i, df7.loc[i, 'car_age_ori']] )
            #print("2 %s " % df7.loc[i, df7.loc[i, 'car_age_current']] )
            #print("3 %s " % df7.loc[i, 'caractualprice'] )
            #print("555 %s " % type(df7.loc[i, df7.loc[i, 'car_age_ori']]) )
            #print("666 %s " % type(df7.loc[i, df7.loc[i, 'car_age_current']]) )
            #print("777 %s " % type(df7.loc[i, 'caractualprice']) )
    ...
    if (today.day == 1):
        ...
        if rerun_flag == "yes":
            with engine.begin() as conn:
                sql_script = (""" DELETE FROM `b_sc_acc` WHERE pos_dt = date('%s') and app_source = 'cup_yixin_sh' and product='car_loan' """% yesterday.strftime("%Y-%m-%d") )
                print("Time:", datetime.datetime.now().strftime("%H:%M:%S"))
                print(sql_script)
                conn.execute(sql_script)
                print("Time:", datetime.datetime.now().strftime("%H:%M:%S"))
                
                sql_script = (""" DELETE FROM `cup_yixin_sh car_loan_bscore_factor` WHERE pos_dt = date('%s') """% yesterday.strftime("%Y-%m-%d") )
                print("Time:", datetime.datetime.now().strftime("%H:%M:%S"))
                print(sql_script)
                conn.execute(sql_script)
                print("Time:", datetime.datetime.now().strftime("%H:%M:%S"))
                
                sql_script = (""" DELETE FROM `cup_yixin_sh car_loan b champ 20210609` WHERE pos_dt = date('%s') """% (yesterday.strftime("%Y-%m-%d")) )
                print("Time:", datetime.datetime.now().strftime("%H:%M:%S"))
                print(sql_script)
                conn.execute(sql_script)
                print("Time:", datetime.datetime.now().strftime("%H:%M:%S"))
        ...
        if not json_temp.empty:
            ...
            json_temp["app_source"] = "cup_yixin_sh"
            ...
            with engine.begin() as conn:
                ...
                print('before cup_yixin_sh car_loan_bscore_factor')
                df10.to_sql(name='cup_yixin_sh car_loan_bscore_factor', con=conn, chunksize=10000, if_exists='append', index=False, method='multi')#'replace'
                print("tables: b_sc_acc, cup_yixin_sh car_loan_bscore_factor ", table_name)

    print("================================")
    print("Time:", datetime.datetime.now().strftime("%H:%M:%S"))
    print("E04_3_1_factor_cup_yixin_sh.py: Completed")
    print("================================")
    running_flag = False
    thr1.join()
    sys.exit(0)
    ...
    print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
    print("Time:", datetime.datetime.now().strftime("%H:%M:%S"))
    strLog = "Failed to process E04_3_1_factor_cup_yixin_sh.py {}".format(error)
    print(strLog)
    print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
    running_flag = False
    thr1.join()
